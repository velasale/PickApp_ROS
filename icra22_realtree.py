...
class AppleProxyExperiment(object):
    ...
    def add_angular_noise(self, noise_range):
        ...
        print('angular noises = ', roll_noise_rad, pitch_noise_rad, yaw_noise_rad)

        # --- Step2: Obtain the current RPY angles, add noise and convert
        roll, pitch, yaw = euler_from_quaternion(quaternion)

        roll = roll + roll_noise_rad
        pitch = pitch + pitch_noise_rad
        yaw = yaw + yaw_noise_rad
    ...
    def go_to_pick_apple(self):
        ...
        delta_z = travel * dz / dxyz
        delta_x = travel * dx / dxyz
        delta_y = travel * dy / dxyz

        print(dx, dy, dz)

        # Pose Position
        pose_goal.position.x = self.apple_pos_x + delta_x
        pose_goal.position.y = self.apple_pos_y + delta_y
        pose_goal.position.z = self.apple_pos_z + delta_z
    ...
    def new_go_to_pick_apple(self):
        ...
        # second_pose.position.x = second_pose.position.x + 0 * (random() * cartesian_noise - cartesian_noise / 2)
        # second_pose.position.y = second_pose.position.y + 0 * (random() * cartesian_noise - cartesian_noise / 2)
    ...
    def noise_toString(self):
        ...
        output = "_noiseX_" + str(num_formatter.format(self.percent_noise_X)) + "_noiseY_" + str(
            ...
            num_formatter.format(self.initial_roll_angle)) + \
                 "_PitchNoise_" + str(num_formatter.format(self.pitch_noise_deg)) + "_RollNoise_" + str(
            num_formatter.format(self.roll_noise_deg)) + "_YawNoise_" + str(
            num_formatter.format(self.yaw_noise_deg))
        return output
...
def main():
    try:
        ...
        for i in range(apples_to_pick):
            print("====== Step 1: Free Drive the Arm towards an apple, and place 10cm from it =========")
            raw_input()
            #apple_proxy_experiment.go_to_starting_position(i)
            print("\n Pal in fron of the apple")
