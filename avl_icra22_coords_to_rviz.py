...
class AppleProxyExperiment(object):
    ...
    """
        ...
        caption.pose.position.z = z + 0.06
...
def main():
    try:
        ...
        for i in range(apples):
            ...
            # Step 1: Place Apple
            diam = float(apple_coords[i][4])
            a = float(apple_coords[i][5])
            b = float(apple_coords[i][6])
            c = float(apple_coords[i][7])
            # print('The apples coordinates are')
            # print(apple_coords[i][5], apple_coords[i][6], apple_coords[i][7])

            apple_proxy_experiment.place_marker(1, 0, 0, 0.5, a, b, c, diam)

            # Step 2: Place Stem
            p1 = apple_coords[i][8]     # Calix of the apple
            p2 = apple_coords[i][9]     # A point in the stem
            ...
            # Step 3: Place Number
            apple_proxy_experiment.place_marker_text(a, b, c, 0.05, str(i+1))
