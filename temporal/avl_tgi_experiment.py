...
class AppleProxyExperiment(object):
  ...
  def add_angular_noise(self, noise_range):
    ...
    adjust_x = self.gripper_height * math.sin(pitch_noise_rad) + self.delta_z*math.tan(pitch_noise_rad)
    pose_goal.position.x = +0.47 + adjust_x

    adjust_z = self.gripper_height*(1 - math.cos(pitch_noise_rad))
    pose_goal.position.z = +0.555 - adjust_z
  ...
  def go_to_pick_apple(self):
    ...
    pose_goal.position.y = -0.012
    pose_goal.position.z = +0.555 - self.delta_z - self.gripper_height*(1 - math.cos(self.pitch_noise_deg*pi/180))
...
def main():
  try:
    ...
    for i in range(apples_to_pick):
      ...
      raw_input()
      appleProxyExperiment.add_angular_noise(30)
