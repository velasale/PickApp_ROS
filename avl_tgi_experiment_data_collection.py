...
class AppleProxyExperiment(object):
  ...
  def __init__(self):
    ...
    self.initial_pitch = pi/4   # 45deg pitch
    #self.initial_pitch = 3*pi / 8  # 30deg pitch
  ...
  def go_to_starting_position(self):
    ...
    pose_goal.position.x = + 0.47 - (self.gripper_height + self.delta_z) * math.cos(pitch_angle)
    pose_goal.position.y = -0.012
    pose_goal.position.z = +0.565 - (self.gripper_height + self.delta_z)*(1-math.sin(pitch_angle))
  ...
  def add_angular_noise(self, noise_range,angle_publisher):
    move_group = self.move_group
    pose_goal = self.move_group.get_current_pose().pose
    ...
    publisher.layout.dim[0].stride = 1
    publisher.data = quaternion

    angle_publisher.publish(publisher)
  ...
  def go_to_pick_apple(self):
    ...
    ## Pose Position
    pose_goal.position.x = self.pos_x + self.delta_z*math.cos(self.total_pitch)
    pose_goal.position.y = -0.012
    pose_goal.position.z = self.pos_z - self.delta_z*math.sin(self.total_pitch)
...
def main():
  label_publisher = rospy.Publisher("/deep_learning_label", Float32, queue_size=10)
  angle_publisher = rospy.Publisher("/deep_learning_quaternion",Float32MultiArray,queue_size=10)
  ...
  try:
    ...
    for i in range(apples_to_pick):
      ...
      raw_input()
      label_publisher.publish(float(0))
      appleProxyExperiment.go_to_starting_position()
      print("Ideal Starting Position reached")

      print "=========== Press 'Enter' to add NOISE to the ideal starting position and pose ==========="
      raw_input()
      appleProxyExperiment.add_angular_noise(44,angle_publisher)
      ...
      print('press enter when the hand has closed')
      raw_input()
      label_publisher.publish(float(1))

      print "============ Press 'Enter' to retrieve normally =========================================="
      raw_input()
      appleProxyExperiment.retrieve()
      print ("Apple No.", i)
      s_or_f = input('success or failure? 2 for success 3 for failure',)
      label_publisher.publish(float(s_or_f))
      # Time in seconds
      time.sleep(1)
